$(function() {

    //input-field focus event listener
    if (document.getElementById("subscribe")) {
        document.getElementById("subscribe").onfocus = function () {
            document.getElementById("subscribe").parentNode.classList.add('input--filled');
            document.getElementById('submessage').style.display = 'none';
        };
        document.getElementById("subscribe").onblur = function () {
            var msgBox = document.getElementById('subscribe');
            var box = document.getElementById('submessage');
            if (msgBox.value == "") {
                msgBox.parentNode.classList.remove('input--filled');
                box.style.display = 'none';
            }
            else {
                var re = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
                var validMail = re.test(msgBox.value);
                if (!validMail) {
                    box.className = 'error-email';
                    box.innerHTML = '<i class="fa fa-times-circle"></i> Please enter a valid email';
                    box.style.display = 'block';
                }
            }
        };
        
        //suppressing the return key default behaviour on the newsletter form
        $('#subform').bind("keypress", function (e) {
            if (e.keyCode == 13) {
                var msgBox = document.getElementById('subscribe');
                var box = document.getElementById('submessage');
                var re = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
                var validMail = re.test(msgBox.value);
                e.preventDefault();
                submitNewsletter();
                if (!validMail) {
                    box.className = 'error-email';
                    box.innerHTML = '<i class="fa fa-times-circle"></i> Please enter a valid email';
                    box.style.display = 'block';
                }
                else {
                    submitNewsletter();
                }
            }
        });
    }
    if (document.getElementById("contact-us")) {
        var msgBox1 = document.getElementById('firstmessage');
        var box1 = document.getElementById('firstName');
        var msgBox2 = document.getElementById('lastmessage');
        var box2 = document.getElementById('lastName');
        var msgBox3 = document.getElementById('emailmessage');
        var box3 = document.getElementById('email');
        var box4 = document.getElementById('message');
        box1.onfocus = function () {
            box1.parentNode.classList.add('input--filled');
            msgBox1.style.visibility = 'hidden';
        };

        box1.onblur = function () {
            if (box1.value == "") {
                box1.parentNode.classList.remove('input--filled');
                msgBox1.style.visibility = 'hidden';
            }
        };

        box2.onfocus = function () {
            box2.parentNode.classList.add('input--filled');
            msgBox2.style.visibility = 'hidden';
        };

        box2.onblur = function () {
            if (box2.value == "") {
                box2.parentNode.classList.remove('input--filled');
                msgBox2.style.visibility = 'hidden';
            }
        };

        box3.onfocus = function () {
            box3.parentNode.classList.add('input--filled');
            msgBox3.style.visibility = 'hidden';
        };

        box3.onblur = function () {
            if (box3.value == "") {
                box3.parentNode.classList.remove('input--filled');
                msgBox3.style.visibility = 'hidden';
            }
            else {
                var re = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i;
                var validMail = re.test(box3.value);
                if (!validMail) {
                    msgBox3.className = 'error-contact';
                    msgBox3.innerHTML = '<i class="fa fa-times-circle"></i> Please enter a valid email';
                    msgBox3.style.visibility = 'visible';
                }
            }
        };

        box4.onfocus = function () {
            box4.parentNode.classList.add('input--filled');
            msgBox4.style.visibility = 'hidden';
        };

        box4.onblur = function () {
            if (box4.value == "") {
                box4.parentNode.classList.remove('input--filled');
                msgBox4.style.visibility = 'hidden';
            }
        };
    }


//overlay menu event listener
$('#toggle').click(function () {
    $(this).toggleClass('active');
    $('#overlay').toggleClass('open');
});
})
;

function submitNewsletter() {
    //handles the newsletter subcription on the home page
    if ($("#url").val() != "") {
        $.ajax({
            method: "POST",
            url: "https://alpha.veruscript.com/api/newsletter/subscribe",
            data: {email: $("#subscribe").val()}

        })
            .done(function (JSmsg) {
                var box = document.getElementById('submessage');
                box.className = 'confirm';
                box.innerHTML = '<i class="fa fa-check-circle"></i> Thank you for subscribing to our newsletter';
                box.style.display = 'block';
            })
            .fail(function (jqXHR, textStatus) {
                var box = document.getElementById('submessage');
                console.log(JSON.stringify(jqXHR), textStatus);
                var msg = JSON.stringify(jqXHR);
                if (msg.indexOf('List_AlreadySubscribed') != -1) {
                    box.className = 'error-already';
                    box.innerHTML = '<i class="fa fa-times-circle"></i> Your email is already in our database';
                    box.style.display = 'block';
                }
                else {
                    box.className = 'error-sent';
                    box.innerHTML = '<i class="fa fa-exclamation-triangle"></i> We are sorry, but a problem has occurred; please, try again later';
                    box.style.display = 'block';
                }
            });
    }
}

function contactUs() {
    //handles the newsletter subcription on the home page

    var ok = true;
    var msgBox1 = document.getElementById('firstmessage');
    var box1 = document.getElementById('firstName');
    if (box1.value == "") {
        ok = false;
        msgBox1.className = 'error-contact';
        msgBox1.innerHTML = '<i class="fa fa-times-circle"></i> Please enter your First Name';
        msgBox1.style.visibility = 'visible';
    }
    var msgBox2 = document.getElementById('lastmessage');
    var box2 = document.getElementById('lastName');
    if (box2.value == "") {
        ok = false;
        msgBox2.className = 'error-contact';
        msgBox2.innerHTML = '<i class="fa fa-times-circle"></i> Please enter your Surname';
        msgBox2.style.visibility = 'visible';
    }
    var msgBox3 = document.getElementById('emailmessage');
    var box3 = document.getElementById('email');
    if (box3.value == "") {
        ok = false;
        msgBox3.className = 'error-contact';
        msgBox3.innerHTML = '<i class="fa fa-times-circle"></i> Please enter your valid Email';
        msgBox3.style.visibility = 'visible';
    }
    var msgBox4 = document.getElementById('messagemessage');
    var box4 = document.getElementById('message');
    if (box4.value == "") {
        ok = false;
        msgBox4.className = 'error-contact textmessage';
        msgBox4.innerHTML = '<i class="fa fa-times-circle"></i> Please write a message';
        msgBox4.style.visibility = 'visible';
    }
    if (ok && $("#url").val() != "") {
        var btn=document.getElementById('button-contact');
        btn.innerHTML = "";
        $.ajax({
            method: "POST",
            url: "../emailSender.php",
            data: {
                firstName: $("#firstName").val(),
                lastName: $("#lastName").val(),
                email: $("#email").val(),
                message: $("#message").val()
            }

        })
            .done(function (msg) {
                var box = document.getElementById('messagemessage');
                if (msg == "") {
                    box.className = 'confirm';
                    box.innerHTML = '<i class="fa fa-check-circle"></i> Thank you for contacting us';
                    box.style.visibility = 'visible';
                }
                else {
                    box.className = 'error-contact';
                    box.innerHTML = '<i class="fa fa-exclamation-triangle"></i> We are sorry, but a problem has occurred; please, try again later';
                    box.style.visibility = 'visible';
                }
            })
            .fail(function (jqXHR, textStatus) {
                var box = document.getElementById('messagemessage');
                msg = '';
                box.className = 'error-contact';
                box.innerHTML = '<i class="fa fa-exclamation-triangle"></i> We are sorry, but a problem has occurred; please, try again later';
                box.style.visibility = 'visible';
            });
    }
}